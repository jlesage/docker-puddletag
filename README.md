...
services:
  puddletag:
    image: jlesage/puddletag
    ports:
      - "5800:5800"
    volumes:
      - "/docker/appdata/puddletag:/config:rw"
...
[Releases]: https://github.com/jlesage/docker-puddletag/releases
[Docker Hub]: https://hub.docker.com/r/jlesage/puddletag/tags
...
container, it would proxy all HTTP requests for `puddletag.domain.tld` to
...
server {
	...
	server_name puddletag.domain.tld;

	location / {
		proxy_pass http://docker-puddletag;
...
server {
	...
	location = /puddletag {return 301 $scheme://$http_host/puddletag/;}
	location /puddletag/ {
		proxy_pass http://docker-puddletag/;
		proxy_http_version 1.1;
		proxy_set_header Upgrade $http_upgrade;
...
| **Application version** label | Displays the version of puddletag integrated into Docker image. |
...
Any desktop notification generated by puddletag is forwarded to the
